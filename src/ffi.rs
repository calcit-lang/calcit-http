...
pub const ASYNC_TASK_REQUIRES_RESPONSE: u32 = 4;
...
pub unsafe fn copy_task_descriptor(value: *const CalcitFfiAsyncTaskV1) -> Result<CalcitFfiAsyncTaskV1, i32> {
  ...
  // SAFETY: the validated size covers every v1 field; unaligned reads keep the
  // adapter robust for foreign C callers.
...
pub fn encode_failure(message: impl Into<String>) -> Vec<u8> {
  encode_edn(&Edn::str(message.into())).unwrap_or_else(|_| b"|failed-to-encode-http-error".to_vec())
}
...
pub unsafe fn run_buffer_adapter(
  ...
  match catch_unwind(AssertUnwindSafe(|| {
    ...
    Err(_) => {
      let _ = unsafe { write_output(output, b"HTTP FFI adapter panicked".to_vec()) };
      1
    }
  }
}
...
pub fn enqueue_with_backpressure(
  ...
  loop {
    let status = unsafe { enqueue(host.context, task.handle, kind, response_handle, payload.as_ptr(), payload.len()) };
