...
- Export `abi_version() -> String` and `edn_version() -> String`; the latter returns `cirru_edn::version().to_string()`.
- Rust 2024 exports use `#[unsafe(no_mangle)]`.
- Preserve the `serve_http` callback ABI and the request/response map keys documented in `README.md`.
...
For the `server` entry, run it once and verify the response with `curl http://127.0.0.1:4000/` in a separate process when the entry is intended to stay running. Confirm that the dylib copied into `dylibs/` is the artifact just built.
